...
jQuery(document).ready(function($){$(window).resize(function(){$('.player-container').each(function(){if($(this).width()>580){$(this).addClass('player-wide');}else{$(this).removeClass('player-wide');}});});});
...
var $yt_links = $("a[href*='http://www.youtube.com/watch']"); // Selects all links that have an href value pointing at youtube
$.each($yt_links, function(i) { // Iterate through the links creating a new media player for each
var $holder = $(‘<span/>’); // Create a container for the media player
$(this).parent().replaceWith($holder); // Replace the parent element of the link (a <p> tag) with the holder
var $caption_link = $(this).siblings('.captions'); // Get a reference to any sibling captions links that exist
...
var link = $(this).attr('href').split("=")[1]; // Extract the video id from the YouTube url in the link
